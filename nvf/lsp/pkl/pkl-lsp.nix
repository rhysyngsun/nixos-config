...
let
  ...
  renamedSetupOpts =
    batchRenameOptions [ "vim" "lsp" "pkl-lsp" ] [ "vim" "lsp" "pkl-lsp" "setupOpts" ]
      migrationTable;
in
{
  imports = renamedSetupOpts;

  options.vim.lsp.pkl-lsp = {
    enable = mkEnableOption ''
      pkl-lsp, plugin to use Neovim as a language server to inject LSP diagnostics,
      code actions, and more via Lua.
    '';

    setupOpts = mkPluginSetupOption "pkl-lsp" {
      debug = mkEnableOption ''
        debugging information for pkl-lsp.

        Displays all possible log messages and writes them to the pkl-lsp log,
      ...
      diagnostics_format = mkOption {
        ...
        default = "[#{m}] #{s} (#{c})";
        description = ''
          Sets the default format used for diagnostics. pkl-lsp will replace th
          e following special components with the relevant diagnostic information:

          * `#{m}`: message
          * `#{s}`: source name (defaults to pkl-lsp if not specified)
          * `#{c}`: code (if available)
        '';
      };
      ...
      sources = mkOption {
        ...
        default = null;
        description = "Sources for pkl-lsp to register";
      };
      ...
      on_attach = mkOption {
        ...
        description = ''
          Defines an on_attach callback to run whenever pkl-lsp attaches to a buffer.
        '';
      };
    };
  };
