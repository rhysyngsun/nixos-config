...
in
{
  options.progams.nvf.settings.vim.languages.jinja = {
    ...
    lsp = {
      enable = mkEnableOption "Jinja LSP support" // {
        default = cfg.vim.lsp.enable;
      };
    ...
    (mkIf jinja_cfg.lsp.enable {
      ...
      vim.lsp.lspconfig.sources.jinja-lsp = ''
        lspconfig.jinja_lsp.setup{
          ...
          cmd = ${
            if isList jinja_cfg.lsp.package then
              expToLua jinja_cfg.lsp.package
            else
              ''{"${jinja_cfg.lsp.package}/bin/jinja-lsp"}''
          }
        }
      '';
    })
