# Vendored from home-manager, `modules/programs/pi-coding-agent.nix`.
#
# Copied verbatim at rev c53d643b3737e2fcd04e6cb3b3580ef50b2087a0 (home-manager 26.11).
# This module does not exist in `release-26.05`, which `flake.nix` pins, so the
# home configs carry it themselves - `modules/programs/programs-ai-pi-local.nix`
# consumes `programs.pi-coding-agent` and needs the option to be declared.
#
# Its only home-manager-internal dependency is `lib.hm.strings.isPathLike`,
# which 26.05 does provide (`modules/lib/strings.nix`).
#
# **Do not edit.** To refresh, re-copy from a newer home-manager and re-check
# that dependency. Drop this file and its two `imports` entries in
# `modules/home/home-*.nix` once the pinned home-manager ships the module.
{
  config,
  lib,
  pkgs,
