{ ... }:
{
  flake.modules.homeManager.programs-claude-code-personal =
    {
      config,
      lib,
      pkgs,
    ...
    let
      ...
      cfg = config.programs.claude-code-personal;
    ...
    in
    {
      ...
      imports = [
        (mkChangedOptionModule
          [ "programs" "claude-code-personal" "memory" "text" ]
          [ "programs" "claude-code-personal" "context" ]
          (config: lib.getAttrFromPath [ "programs" "claude-code-personal" "memory" "text" ] config)
        )
        (mkChangedOptionModule
          [ "programs" "claude-code-personal" "memory" "source" ]
          [ "programs" "claude-code-personal" "context" ]
          (config: lib.getAttrFromPath [ "programs" "claude-code-personal" "memory" "source" ] config)
        )
        (mkChangedOptionModule
          [ "programs" "claude-code-personal" "skillsDir" ]
          [ "programs" "claude-code-personal" "skills" ]
          (config: lib.getAttrFromPath [ "programs" "claude-code-personal" "skillsDir" ] config)
        )
      ];

      options.programs.claude-code-personal = {
        enable = lib.mkEnableOption "Claude Code, Anthropic's official CLI (personal profile, ~/.claude-personal)";
        ...
        enableMcpIntegration = mkOption {
          ...
          description = ''
            ...
            with {option}`programs.claude-code-personal.mcpServers`, with Claude Code servers
            taking precedence.
          '';
        };
        ...
        agents = mkContentOption {
          description = ''
            ...
            Agents are stored in .claude-personal/agents/ directory.
          '';
          example = literalExpression ''
            {
              code-reviewer = '''
        ...
        commands = mkContentOption {
          description = ''
            ...
            Commands are stored in .claude-personal/commands/ directory.
          '';
          example = literalExpression ''
            {
              changelog = '''
        ...
        hooks = mkOption {
          ...
          description = ''
            ...
            Hooks are stored in .claude-personal/hooks/ directory.
          '';
          example = {
            pre-edit = ''
              #!/usr/bin/env bash
        ...
        rules = mkContentOption {
          description = ''
            ...
            Rules are stored in .claude-personal/rules/ directory.
            All markdown files in .claude-personal/rules/ are automatically loaded as project memory.
          '';
          example = literalExpression ''
            {
              code-style = '''
        ...
        rulesDir = mkDirOption {
          description = ''
            ...
            Rule files from this directory will be symlinked to .claude-personal/rules/.
        ...
        agentsDir = mkDirOption {
          description = ''
            ...
            Agent files from this directory will be symlinked to .claude-personal/agents/.
          '';
        ...
        commandsDir = mkDirOption {
          description = ''
            ...
            Command files from this directory will be symlinked to .claude-personal/commands/.
          '';
        ...
        hooksDir = mkDirOption {
          description = ''
            ...
            Hook files from this directory will be symlinked to .claude-personal/hooks/.
          '';
        ...
        outputStyles = mkContentOption {
          description = ''
            ...
            The value is either:
              - Inline content as a string
              - A path to a file
            In both cases, the contents will be written to .claude-personal/output-styles/<name>.md
          '';
          example = literalExpression ''
            {
        ...
        skills = mkOption {
          ...
          description = ''
            ...
            - Inline content as a string (creates .claude-personal/skills/<name>/SKILL.md)
            - A path to a file (creates .claude-personal/skills/<name>/SKILL.md)
            - A path to a directory (creates .claude-personal/skills/<name>/ with all files)
            ...
            symlinked to {file}`~/.claude-personal/skills/`.
          '';
          example = literalExpression ''
            {
      ...
      config =
        let
          ...
          mkMarkdownEntries =
            subdir: attrs:
            lib.mapAttrs' (
              name: content: nameValuePair ".claude-personal/${subdir}/${name}.md" (mkSourceEntry content)
            ) attrs;

          mkTextEntries =
            subdir: attrs:
            lib.mapAttrs' (
              name: content: nameValuePair ".claude-personal/${subdir}/${name}" { text = content; }
            ) attrs;

          mkRecursiveDirAttrs =
            subdir: dir:
            optionalAttrs (dir != null) {
              ".claude-personal/${subdir}" = {
                source = dir;
                recursive = true;
              };
            };
          ...
          mkSkillEntry =
            ...
            if isPathLikeContent content && lib.pathIsDirectory content then
              nameValuePair ".claude-personal/skills/${name}" {
                source = content;
                recursive = true;
              }
            else
              nameValuePair ".claude-personal/skills/${name}/SKILL.md" (
          ...
          assertions =
            let
              ...
              mkExclusiveAssertion = inline: {
                ...
                message = "Cannot specify both `programs.claude-code-personal.${inline}` and `programs.claude-code-personal.${inline}Dir`";
              };
            in
            [
              {
                assertion =
                  ...
                  || cfg.package != null;
                message = "`programs.claude-code-personal.package` cannot be null when `mcpServers`, `lspServers`, `enableMcpIntegration`, or `plugins` is configured";
              }
              {
                assertion = !lib.isPath cfg.skills || lib.pathIsDirectory cfg.skills;
                message = "`programs.claude-code-personal.skills` must be a directory when set to a path";
              }
            ]
            ++ map mkExclusiveAssertion exclusiveInlineDirNames;

          programs.claude-code-personal.finalPackage =
            let
              ...
              pluginDir = pkgs.runCommand "claude-code-personal-hm-plugin" { } (
                ''
                  install -Dm644 ${
                    jsonFormat.generate "claude-code-plugin.json" {
                      name = "claude-code-personal-home-manager";
                    }
          ...
          home = {
            ...
            file = lib.mkMerge [
              (lib.mkIf (cfg.settings != { } || cfg.marketplaces != { }) {
                ".claude-personal/settings.json".source = jsonFormat.generate "claude-code-personal-settings.json" (
                  cfg.settings
                  // {
              ...
              (
                if lib.isPath cfg.context then
                  {
                    ".claude-personal/CLAUDE.md".source = cfg.context;
                  }
                else
                  (lib.mkIf (cfg.context != "") {
                    ".claude-personal/CLAUDE.md".text = cfg.context;
                  })
              )
              (lib.mkIf (cfg.marketplaces != { }) {
                ".claude-personal/plugins/known_marketplaces.json".source =
                  jsonFormat.generate "claude-code-personal-known-marketplaces.json" (
